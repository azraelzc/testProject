...
public class QuickSort : SortBase
{
	...
	void Start () {
        ...
        print("result", (new QuickSort().sort(a)));
    }
    ...
    private static void QuickSortFunction(ref int[] array, int low, int high)
    {
        try
        {
            ...
            if (low < high)
            {
                ...
                QuickSortFunction(ref array, low, keyValuePosition - 1);    //递归调用，快排划分出来的左区间
                QuickSortFunction(ref array, keyValuePosition + 1, high);   //递归调用，快排划分出来的右区间
            }
        }
        catch (Exception ex)
        {
    ...
    {
        QuickSortFunction(ref a, 0, a.Length - 1);
        return a;
    }
}
