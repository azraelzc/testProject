 ...
 */
public class GravityFlyYolk : MonoBehaviour {
    ...
    bool isFly = false;
    void Start() {
        _StartPos = new Vector2(pointA.x, pointA.z);
        _EndPos = new Vector2(pointB.x, pointB.z);
        _Time = time;
        _IsUp = isUp;
    ...
    void Update() {
        if (isFly) {
            float angle = GetAngle(_CurrentVec, Vector2.left);
            _CurrentVec += _GravityForce * Time.deltaTime;
            Vector3 targetVec = _CurrentVec * Time.deltaTime;
            Vector3 v = new Vector3(targetVec.x, 0, targetVec.y);
